...
- Name: tests/app.spec.js >> Resolve & Delete >> should resolve prediction as wrong
- Location: tests/app.spec.js:267:1
...
Call log:
  - waiting for locator('.resolve-correct')
    ...
    - generic [ref=e10]:
      ...
      - generic [ref=e12]:
        - generic [ref=e13]:
          ...
          - button "Pending" [ref=e15] [cursor=pointer]
          - button "Resolved" [ref=e16] [cursor=pointer]
        - button "+ Stock" [ref=e17] [cursor=pointer]
        - button "+ Custom" [ref=e18] [cursor=pointer]
    - generic [ref=e19]:
      - generic [ref=e20]:
        - generic [ref=e21]: "0"
      ...
      - generic [ref=e26]:
        - generic [ref=e27]: "0"
        - generic [ref=e28]: Correct
      - generic [ref=e29]:
        - generic [ref=e30]: "0"
        - generic [ref=e31]: Wrong
      - generic [ref=e32]:
        - generic [ref=e33]: —
        - generic [ref=e34]: Accuracy
    - generic [ref=e35]:
      - generic [ref=e36]: 🔮
      - paragraph [ref=e37]: No predictions yet. Make your first call.
```

# Test source

```ts
  74  |     await page.click('button:has-text("+ Custom")');
  75  |     await expect(page.locator('#newModal')).toHaveClass(/show/);
  76  |     await expect(page.locator('#customForm')).toBeVisible();
  77  |     await expect(page.locator('#stockForm')).toBeHidden();
  78  |   });
  79  | 
  80  |   test('modal close button should work', async ({ page }) => {
  81  |     await page.click('button:has-text("+ Stock")');
  82  |     await expect(page.locator('#newModal')).toHaveClass(/show/);
  83  |     await page.click('#newModal .modal button:has-text("✕")');
  84  |     await expect(page.locator('#newModal')).not.toHaveClass(/show/);
  85  |   });
  86  | 
  87  |   test('modal should close on overlay click', async ({ page }) => {
  88  |     await page.click('button:has-text("+ Stock")');
  89  |     await expect(page.locator('#newModal')).toHaveClass(/show/);
  90  |     await page.click('#newModal', { position: { x: 5, y: 5 } });
  91  |     await expect(page.locator('#newModal')).not.toHaveClass(/show/);
  92  |   });
  93  | });
  94  | 
  95  | test.describe('Stock Prediction Flow', () => {
  96  |   test.beforeEach(async ({ page }) => {
  97  |     await page.goto(BASE);
  98  |     // Clear localStorage
  99  |     await page.evaluate(() => localStorage.clear());
  100 |     await page.click('.hero-cta .btn-primary');
  101 |   });
  102 | 
